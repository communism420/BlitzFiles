...
object LibSuFileServiceLauncher {
    ...
    fun isSuAvailable(): Boolean {
        ...
        try {
            waitForExitCompat(process, SU_PROBE_TIMEOUT_MILLIS)
            return true
        } finally {
            process.destroy()
    ...
    /**
     * Timed Process.waitFor() requires API 26, while BlitzFiles still supports API 23.
     */
    private fun waitForExitCompat(process: Process, timeoutMillis: Long) {
        val deadlineNanos =
        ...
        while (true) {
            ...
            val sleepMillis = TimeUnit.NANOSECONDS.toMillis(remainingNanos)
                .coerceIn(1L, SU_PROBE_POLL_MILLIS)
            try {
    ...
    private const val SU_PROBE_TIMEOUT_MILLIS = 1_000L
    private const val SU_PROBE_POLL_MILLIS = 25L
}
