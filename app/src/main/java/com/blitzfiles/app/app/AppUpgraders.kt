...
import com.blitzfiles.app.compat.PreferenceManagerCompat
import com.blitzfiles.app.compat.getDescriptionCompat
import com.blitzfiles.app.compat.readBooleanCompat
import com.blitzfiles.app.compat.writeBooleanCompat
import com.blitzfiles.app.compat.writeParcelableListCompat
import com.blitzfiles.app.file.DocumentTreeUri
import com.blitzfiles.app.file.asExternalStorageUriOrNull
import com.blitzfiles.app.file.displayName
import com.blitzfiles.app.file.storageVolume
import com.blitzfiles.app.filelist.FileSortOptions
import com.blitzfiles.app.navigation.BookmarkDirectory
import com.blitzfiles.app.navigation.StandardDirectorySettings
import com.blitzfiles.app.provider.archive.ArchiveFileSystem
import com.blitzfiles.app.provider.common.ByteString
import com.blitzfiles.app.provider.common.moveToByteString
import com.blitzfiles.app.provider.content.ContentFileSystem
import com.blitzfiles.app.provider.document.DocumentFileSystem
import com.blitzfiles.app.provider.document.resolver.ExternalStorageProviderHacks
import com.blitzfiles.app.provider.linux.LinuxFileSystem
import com.blitzfiles.app.provider.root.RootStrategy
import com.blitzfiles.app.provider.sftp.SftpFileSystem
import com.blitzfiles.app.provider.smb.SmbFileSystem
import com.blitzfiles.app.storage.DocumentTree
import com.blitzfiles.app.storage.FileSystemRoot
import com.blitzfiles.app.storage.PrimaryStorageVolume
import com.blitzfiles.app.util.StableUriParceler
import com.blitzfiles.app.util.asBase64
import com.blitzfiles.app.util.readParcelable
import com.blitzfiles.app.util.readParcelableListCompat
import com.blitzfiles.app.util.toBase64
import com.blitzfiles.app.util.toByteArray
import com.blitzfiles.app.util.use
        ...
        Parcel.obtain().use { newParcel ->
            ...
            Parcel.obtain().use { oldParcel ->
                ...
                repeat(size) {
                    ...
                    oldParcel.readInt()
                    val className = oldParcel.readString()
                    if (className == "com.blitzfiles.app.storage.SmbServer") {
                        newParcel.writeInt(PARCEL_VAL_PARCELABLE)
                        newParcel.writeString("com.blitzfiles.app.storage.SmbServer")
                        ...
                        newParcel.writeString(
                            "com.blitzfiles.app.provider.smb.client.Authority"
                        )
                        ...
                        oldParcel.readString()
                        newParcel.writeString(
                            "com.blitzfiles.app.provider.smb.client.Authentication"
                        )
...
private fun migratePath1_4_0(oldParcel: Parcel, newParcel: Parcel) {
    ...
    when (className) {
        ...
        "com.blitzfiles.app.provider.sftp.SftpPath" ->
            newParcel.writeParcelable(oldParcel.readParcelable<SftpFileSystem>(), 0)
        "com.blitzfiles.app.provider.smb.SmbPath" ->
            ...
            Parcel.obtain().use { oldParcel ->
                ...
                repeat(size) {
                    ...
                    when (oldParcel.readString()) {
                        "com.blitzfiles.app.storage.SftpServer" -> {
                            newParcel.writeInt(PARCEL_VAL_PARCELABLE)
                            newParcel.writeString("com.blitzfiles.app.storage.SftpServer")
                            val id = oldParcel.readLong()
                            newParcel.writeLong(id)
                            val customName = oldParcel.readString()
                            newParcel.writeString(customName)
                ...
                repeat(size) {
                    ...
                    when (oldParcel.readString()) {
                        "com.blitzfiles.app.storage.SftpServer" -> {
                            ...
                            val authenticationPasswordOrPrivateKey = oldParcel.readString()
                            newParcel.writeString(authenticationPasswordOrPrivateKey)
                            if (authenticationClassName == "com.blitzfiles.app.provider.sftp"
                ...
                Parcel.obtain().use { oldParcel ->
                    ...
                    readWriteLengthPrefixedValue(oldParcel, newParcel) {
                        ...
                        repeat(size) {
                            ...
                            oldParcel.setDataPosition(oldPosition)
                            when (className) {
                                "com.blitzfiles.app.storage.DocumentManagerShortcut" -> {
                                    newParcel.writeInt(oldParcel.readInt())
                                    readWriteLengthPrefixedValue(oldParcel, newParcel) {
                                        oldParcel.readString()
                                        newParcel.writeString(
                                            "com.blitzfiles.app.storage" +
