...
fun upgradeApp() {
    val isFreshInstall = isFreshInstallation()
    InitialIndexingCoordinator.initializeInstallEligibility(isFreshInstall)
    val previousVersionCode = lastVersionCode
    upgradeAppFrom(previousVersionCode)
    lastVersionCode = VERSION_CODE_LATEST
}
...
private fun isFreshInstallation(): Boolean {
    ...
    // Auto Backup restores preferences but not the no-backup filename database. On a restored
    // installation firstInstallTime and lastUpdateTime describe the new package installation, so
    // treat it as fresh even when restored default preferences are already present.
