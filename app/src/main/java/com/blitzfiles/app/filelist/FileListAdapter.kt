...
import com.blitzfiles.app.coil.AppIconPackageName
import com.blitzfiles.app.compat.foregroundCompat
import com.blitzfiles.app.compat.getDrawableCompat
...
import com.blitzfiles.app.ui.CheckableItemBackground
import com.blitzfiles.app.util.isMaterial3Theme
...
class FileListAdapter(
    ...
    @Deprecated(
        "",
        ReplaceWith("replaceListAndIsSearching(list, isSearching, searchHighlightQuery)")
    )
    ...
    fun replaceListAndIsSearching(
        ...
        val highlightChanged = this.searchHighlightQuery != searchHighlightQuery
        // Query changes can replace an unbounded recursive result with a tiny provisional list.
        // Clearing avoids running synchronous DiffUtil over tens of thousands of stale rows on the
        // main thread while the user is typing.
        val clear = this.isSearching != isSearching || (isSearching && highlightChanged)
        this.isSearching = isSearching
        this.searchHighlightQuery = searchHighlightQuery
        val sortedList = if (!isSearching) list.sortedWith(sortOptions.createComparator()) else list
        super.replace(sortedList, clear)
        rebuildFilePositionMap()
        if (highlightChanged && !clear && itemCount > 0) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int, payloads: List<Any>) {
        ...
        if (payloads.any { payload -> payload === PAYLOAD_SEARCH_HIGHLIGHT_CHANGED }) {
    ...
    override val isAnimationEnabled: Boolean
        get() = !isSearching && Settings.FILE_LIST_ANIMATION.valueCompat

    private fun bindHighlightedName(holder: ViewHolder, file: FileItem) {
        holder.nameText.text = holder.nameText.context.highlightSearchMatches(
            text = file.name,
