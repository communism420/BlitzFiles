...
import com.blitzfiles.app.app.BackgroundActivityStarter
import com.blitzfiles.app.app.mainExecutor
import com.blitzfiles.app.compat.mainExecutorCompat
...
import com.blitzfiles.app.filelist.OpenFileAsDialogActivity
import com.blitzfiles.app.filelist.OpenFileAsDialogFragment
import com.blitzfiles.app.indexing.FileIndexingController
import com.blitzfiles.app.provider.archive.archiveFile
import com.blitzfiles.app.provider.archive.archiver.ArchiveWriter
import com.blitzfiles.app.provider.archive.createArchiveRootPath
import com.blitzfiles.app.provider.archive.isArchivePath
import com.blitzfiles.app.provider.common.ByteString
import com.blitzfiles.app.provider.common.ByteStringBuilder
import com.blitzfiles.app.provider.common.InvalidFileNameException
import com.blitzfiles.app.provider.common.PosixFileModeBit
...
import com.blitzfiles.app.provider.common.ProgressCopyOption
import com.blitzfiles.app.provider.common.ReadOnlyFileSystemException
import com.blitzfiles.app.provider.common.UserActionRequiredException
import com.blitzfiles.app.provider.common.asByteStringListPath
import com.blitzfiles.app.provider.common.copyTo
import com.blitzfiles.app.provider.common.createDirectories
import com.blitzfiles.app.provider.common.createDirectory
import com.blitzfiles.app.provider.common.createFile
import com.blitzfiles.app.provider.common.delete
import com.blitzfiles.app.provider.common.deleteIfExists
import com.blitzfiles.app.provider.common.exists
import com.blitzfiles.app.provider.common.getFileStore
import com.blitzfiles.app.provider.common.getMode
import com.blitzfiles.app.provider.common.getPath
import com.blitzfiles.app.provider.common.isDirectory
import com.blitzfiles.app.provider.common.moveTo
import com.blitzfiles.app.provider.common.newByteChannel
import com.blitzfiles.app.provider.common.newDirectoryStream
import com.blitzfiles.app.provider.common.newOutputStream
import com.blitzfiles.app.provider.common.readAttributes
import com.blitzfiles.app.provider.common.resolveForeign
import com.blitzfiles.app.provider.common.restoreSeLinuxContext
import com.blitzfiles.app.provider.common.setGroup
import com.blitzfiles.app.provider.common.setMode
import com.blitzfiles.app.provider.common.setOwner
import com.blitzfiles.app.provider.common.setSeLinuxContext
import com.blitzfiles.app.provider.common.toByteString
import com.blitzfiles.app.provider.common.toModeString
...
import com.blitzfiles.app.util.toEnumSet
...
class DeleteFileJob(private val paths: List<Path>) : FileJob() {
    @Throws(IOException::class)
    override fun run() {
        val preparedDeletion = FileDeletionStore.prepareDeletion(paths) ?: return
        val deletedPaths = DeletedPathAccumulator()
        try {
            ...
            val actionAllInfo = ActionAllInfo()
            for (path in paths) {
                deleteRecursively(path, transferInfo, actionAllInfo, deletedPaths::add)
                throwIfInterrupted()
            }
        } finally {
            val recordIds = setOf(preparedDeletion.id)
            val confirmedPaths = deletedPaths.snapshot()
            if (!confirmedPaths.isEmpty() || deletedPaths.isOverflowed) {
                val narrowedPaths = if (deletedPaths.isOverflowed) {
                    // A very large flat partial deletion is kept fail-closed under its durable
                    // parent fence rather than retaining unbounded path strings and risking OOM.
                    preparedDeletion.paths
                } else {
                    confirmedPaths
                }
                val protectedPaths = if (
                    narrowedPaths == preparedDeletion.paths ||
                    FileDeletionStore.confirmDeletion(preparedDeletion.id, narrowedPaths)
                ) {
                    narrowedPaths
                } else {
                    // A failed durable narrowing must remain fail-closed. The prepared parent
                    // fence is already on disk and a targeted scan will restore surviving paths.
                    preparedDeletion.paths
                }
                reconcileDeletedIndexPaths(
                    protectedPaths.indexPathPrefixes,
                    recordIds
                )
            } else {
                // Scanning or user interaction failed before the first physical mutation.
    ...
    private fun reconcileDeletedIndexPaths(
        pathPrefixes: Set<String>,
        deletionRecordIds: Set<Long>
    ) {
        if (deletionRecordIds.isEmpty()) {
            return
        }
        if (pathPrefixes.isEmpty()) {
            FileDeletionStore.releaseRecordsWithoutVerification(deletionRecordIds)
            return
        }
        // File jobs use interruption for cancellation. Once the filesystem operation has
        // succeeded, index cleanup must still get a chance to commit before the interrupted flag
        // is restored for the job runner.
        val wasInterrupted = Thread.interrupted()
        try {
            val compactedPathPrefixes = compactPathPrefixes(pathPrefixes)
            val reconciliation = runBlocking {
                FileIndexingController.reconcileDeletedPaths(service, compactedPathPrefixes)
            }
            val hintBatches = FileIndexingController.chunkPathHints(
                reconciliation.incrementalPathHints
            )
            val protectedBatches = hintBatches.mapNotNull { pathHints ->
                FileDeletionStore.createIndexProtection(deletionRecordIds)?.let { token ->
                    ProtectedPathHints(pathHints, token)
                }
            }
            ...
            protectedBatches.forEach { protectedBatch ->
                try {
                    ...
                    // Keep the durable fence retryable. This guards against an older full scan
                    // that may still reinsert metadata read before deletion.
                    FileDeletionStore.completeIndexProtection(
                        protectedBatch.protectionToken,
                        succeeded = false
                    )
            ...
            // Physical deletion must not be reported as failed because optional index maintenance
            // encountered a transient database error. Process tombstones still prevent stale hits.
            error.printStackTrace()
            FileDeletionStore.markRecordsRetryable(deletionRecordIds)
            FileDeletionStore.recordIndexReconciliationFinished()
        } finally {
            if (wasInterrupted) {
...
private data class ProtectedPathHints(
